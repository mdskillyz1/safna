    ...
    <article className={`product-card${compact ? " compact" : ""}`}>
      ...
      <div style={{ display: "grid", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 4 }}>
          {[product.size, product.heatLevel, product.dietary, product.comingSoon ? "Coming soon" : getStockLabel(product.stock)].map((item) => (
            <span
              key={item}
              style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 14 }}>
        ...
        <button className="button yellow" type="button" onClick={() => addItem(product)} disabled={product.stock <= 0 || product.price <= 0}>
          <ShoppingBasket size={18} /> {product.comingSoon || product.price <= 0 ? "Soon" : "Add"}
        </button>
      </div>
